...
Real-time audience feedback system for presentations. Audience members open `controller.html` on their phones and send emoji reactions; reactions appear live on `display.html` (projected). An ESP32 or other presenter device can publish custom events directly to Ably's REST API to trigger effects on the display.
